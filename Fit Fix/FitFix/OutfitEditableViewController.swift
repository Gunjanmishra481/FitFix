...
class OutfitEditableViewController: UIViewController {
    ...
    var savedOutfits: [ClothingItem] = []  // Array to hold saved outfits
    ...
    // Action for the Save Button (Bookmark icon)
    @IBAction func saveButtonTapped(_ sender: UIButton) {
        // Capture screenshot of current outfit screen
        guard let screenshot = captureScreenshot() else {
            let alert = UIAlertController(title: "Error", message: "Failed to capture screenshot.", preferredStyle: .alert)
            alert.addAction(UIAlertAction(title: "OK", style: .default))
            present(alert, animated: true)
        ...
        // Create a ClothingItem from the captured screenshot
        // Removed 'name' argument; assuming we only need 'imageName' and 'category'
        let savedItem = ClothingItem(category: "Outfit", imageName: screenshot)

        // Add the ClothingItem to the savedOutfits array
        savedOutfits.append(savedItem)

        // Redirect to SavedViewController
        let savedVC = storyboard?.instantiateViewController(withIdentifier: "SavedViewController") as! SavedViewController
